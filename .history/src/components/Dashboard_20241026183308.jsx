// import React, { useState } from 'react';

// import Header from './Header';

// const Dashboard = ({ 
//   playlists, 
//   likedSongs, 
//   recentSongs, 
//   mostPlayedSongs, 
//   customPlaylists, 
//   createPlaylist, 
//   deleteSongFromPlaylist 
// }) => {
//   const [newPlaylistName, setNewPlaylistName] = useState('');

//   const handleCreatePlaylist = () => {
//     if (newPlaylistName) {
//       createPlaylist(newPlaylistName);
//       setNewPlaylistName('');
//     }
//   };

//   return (
//     <div>
//       <Header />
      
//       <div className="p-4 bg-gray-900 text-white rounded-lg shadow-lg">
//       <h2 className="text-2xl mb-4">Dashboard</h2>

//       {/* Custom Playlists Section */}
//       <div className="mb-6">
//         <h3 className="text-lg mb-2">Create a Playlist</h3>
//         <input 
//           type="text"
//           value={newPlaylistName}
//           onChange={(e) => setNewPlaylistName(e.target.value)}
//           className="p-2 rounded bg-gray-700 text-white mb-2"
//           placeholder="Enter playlist name"
//         />
//         <button onClick={handleCreatePlaylist} className="bg-blue-600 p-2 rounded">Create Playlist</button>

//         <div className="mt-4">
//           <h3 className="text-lg mb-2">Your Playlists</h3>
//           {customPlaylists.map((playlist, index) => (
//             <div key={index} className="p-2 bg-gray-700 rounded mb-2">
//               <h4 className="text-md">{playlist.name}</h4>
//               <button className="bg-blue-600 p-2 mt-2 rounded">Play Playlist</button>
//             </div>
//           ))}
//         </div>
//       </div>

//       {/* Liked Songs Section */}
//       <div className="mb-6">
//         <h3 className="text-lg mb-2">Liked Songs</h3>
//         {likedSongs.length > 0 ? (
//           <button className="bg-green-600 p-2 rounded">Play Liked Playlist</button>
//         ) : (
//           <p>No liked songs yet.</p>
//         )}
//       </div>

//       {/* Library Section */}
//       <div className="mb-6">
//         <h3 className="text-lg mb-2">Library</h3>
        
//         <div>
//           <h4 className="text-md mb-2">Recently Played</h4>
//           {recentSongs.length > 0 ? recentSongs.map((song, index) => (
//             <div key={index} className="flex justify-between p-2 bg-gray-700 rounded mb-2">
//               <span>{song.name}</span>
//               <button className="text-sm bg-blue-600 p-1 rounded">Play</button>
//             </div>
//           )) : <p>No recent songs.</p>}
//         </div>

//         <div className="mt-4">
//           <h4 className="text-md mb-2">Most Played</h4>
//           {mostPlayedSongs.length > 0 ? mostPlayedSongs.map((song, index) => (
//             <div key={index} className="flex justify-between p-2 bg-gray-700 rounded mb-2">
//               <span>{song.name}</span>
//               <button className="text-sm bg-blue-600 p-1 rounded">Play</button>
//             </div>
//           )) : <p>No most played songs.</p>}
//         </div>
//       </div>
//     </div>
//     </div>
//   );
// };

// export default Dashboard;


import React, { useState } from 'react';
import Header from './Header';
import './Dashboard.css'; // Import your CSS file

const Dashboard = ({ 
  playlists, 
  likedSongs, 
  recentSongs, 
  mostPlayedSongs, 
  customPlaylists, 
  createPlaylist, 
  deleteSongFromPlaylist, 
  addToPlaylist
}) => {
  const [newPlaylistName, setNewPlaylistName] = useState('');
  const [selectedSong, setSelectedSong] = useState(null);
  const [selectedPlaylist, setSelectedPlaylist] = useState(null);
  const [showAddSongModal, setShowAddSongModal] = useState(false);

  const handleCreatePlaylist = () => {
    if (newPlaylistName) {
      createPlaylist(newPlaylistName);
      setNewPlaylistName('');
    }
  };

  const handleAddToPlaylist = () => {
    if (selectedSong && selectedPlaylist) {
      addToPlaylist(selectedSong, selectedPlaylist);
      setSelectedSong(null);
      setSelectedPlaylist(null);
      setShowAddSongModal(false);
    }
  };

  const handleSelectSong = (song) => {
    setSelectedSong(song);
    setShowAddSongModal(true);
  };

  const handleSelectPlaylist = (playlist) => {
    setSelectedPlaylist(playlist);
  };

  return (
    <div className="dashboard-container">
      <Header />

      <div className="dashboard-content">
        {/* Dashboard Left Side */}
        <div className="dashboard-left">
          <h2 className="dashboard-title">Dashboard</h2>

          {/* Custom Playlists Section */}
          <div className="dashboard-section">
            <h3>Create a Playlist</h3>
            <input 
              type="text"
              value={newPlaylistName}
              onChange={(e) => setNewPlaylistName(e.target.value)}
              className="dashboard-input"
              placeholder="Enter playlist name"
            />
            <button onClick={handleCreatePlaylist} className="dashboard-button">Create Playlist</button>

            <div className="mt-4">
              <h3>Your Playlists</h3>
              {customPlaylists.map((playlist, index) => (
                <div key={index} className ="dashboard-playlist">
                  <h4>{playlist.name}</h4>
                  <button className="dashboard-button" onClick={() => handleSelectPlaylist(playlist)}>Select</button>
                </div>
              ))}
            </div>
          </div>

          {/* Liked Songs Section */}
          <div className="dashboard-section">
            <h3>Liked Songs</h3>
            {likedSongs.length > 0 ? (
              <button className="dashboard-button" onClick={() => handleSelectSong(likedSongs)}>Select</button>
            ) : (
              <p>No liked songs yet.</p>
            )}
          </div>

          {/* Library Section */}
          <div className="dashboard-section">
            <h3>Library</h3>
            
            <div>
              <h4>Recently Played</h4>
              {recentSongs.length > 0 ? recentSongs.map((song, index) => (
                <div key={index} className="dashboard-song">
                  <span>{song.name}</span>
                  <button className="dashboard-button" onClick={() => handleSelectSong(song)}>Select</button>
                </div>
              )) : <p>No recent songs.</p>}
            </div>

            <div className="mt-4">
              <h4>Most Played</h4>
              {mostPlayedSongs.length > 0 ? mostPlayedSongs.map((song, index) => (
                <div key={index} className="dashboard-song">
                  <span>{song.name}</span>
                  <button className="dashboard-button" onClick={() => handleSelectSong(song)}>Select</button>
                </div>
              )) : <p>No most played songs.</p>}
            </div>
          </div>
        </div>

        {/* Dashboard Right Side */}
        <div className="dashboard-right">
          {showAddSongModal && (
            <div className="dashboard-modal">
              <h3>Add Song to Playlist</h3>
              <p>Select a playlist to add the song to:</p>
              {customPlaylists.map((playlist, index) => (
                <div key={index} className="dashboard-playlist">
                  <h4>{playlist.name}</h4>
                  <button className="dashboard-button" onClick={() => handleAddToPlaylist()}>Add to Playlist</button>
                </div>
              ))}
              <button className="dashboard-button" onClick={() => setShowAddSongModal(false)}>Cancel</button>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default Dashboard;
